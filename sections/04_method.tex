...
goal is to recover a thermal-to-RGB homography that remains reliable at the scene level, while
withholding a canonical alignment when the evidence does not satisfy the reliability criterion.

This formulation differs from treating registration as a single best-pair problem. In our setting,
many scenes already contain usable pairwise correspondences, but the central challenge is to
convert uneven frame-level estimates into a transform that is geometrically stable, internally
consistent, and acceptable under a reliability-aware criterion.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_method_pipeline.pdf}
\caption{Overview of UAV-TAlign. The framework builds on state-of-the-art pairwise matching, but
...
\subsection{Pairwise Backbone and Reliability-Guided Evidence Selection}
...
At the scene level, UAV-TAlign does not assume that all frames should contribute equally from the
start. It uses a progressive evidence schedule that expands with scene size and can cover all
...
The canonical decision uses a conservative baseline-aware verification rule. A scene is retained
when accepted-frame statistics remain strong, the aggregated homography is stable, severe
baseline-relative outliers remain controlled, and the transform improves gradient-based alignment
beyond configured floors. This final verification step is what separates UAV-TAlign from direct
pairwise use of the same matcher: the framework optimizes for reliable scene-level alignment rather
than merely maximizing the number of matched frames.
